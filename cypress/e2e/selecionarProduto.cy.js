...
describe('Selecionar Produtos', () => {
  ...
  it('Selecionar Sauce Labs Backpack', () => {
    ...
    cy.xpath('/html/body/div[1]/div/div/div[1]/div[2]/div/button')
    ...
    cy.get('span.title')
      .should('have.text', 'Your Cart')      // verifica o título da seção

    cy.get('div.inventory_item_name')
      .should('have.text', 'Sauce Labs Backpack')   // verifica o título do produto
  ...
  massa.array.forEach(({ username, productName, productPrice }) => {
    it(`Selecionar ${productName} - Usuario: ${username}`, () => {
      ...
      cy.get('div.inventory_item_name')
        .should('have.text', productName)   // verifica o título do produto
  ...
  afterEach(() => {
    ...
    cy.get('#logout_sidebar_link', { timeout: 10000 })
      .should('be.visible')         // esperar até que o elemento esteja visível
      // .should('not.be.disabled')   // esperar até que ele esteja visível
